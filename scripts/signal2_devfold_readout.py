...
这份脚本是 ``experiments/signal2_prereg_v2.md`` 的执行器，仅此而已。

**判读阈值不在本脚本里。** 本脚本只产出读数（RankIC、与 Kronos 的秩相关、换手率），
不做任何判定、不打印任何「通过 / 不通过」。阈值与处置规则写在预注册文档里，
且必须在看到本脚本输出**之前**落笔（CLAUDE.md §二：判据先于结果）。

**判据 = v2 §4 准入检查**（``IC₂ > 0 ∧ k⁺ >= 4/7 ∧ ρ(FT) < 0.30``）。
本脚本只产出读数，**准入判定由读取者按 v2 机械执行**；
**不得据本脚本任何输出选择合成规则**——合成规则已由 v2 §4a 先验裁定
（逐日等权秩相加），与本脚本的任何数字无因果关系。
