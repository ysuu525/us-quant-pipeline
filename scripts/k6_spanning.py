...
# 起点提前到 2019-10：idiovol 需 60 日、amihud/hlrange/volshock 需 20 日回看窗
LO, HI = "2019-10-01", "2024-01-05"
EXIT_PCT, NT, TOPN, COST_BP = 0.30, 6, 500, 8.0

CORE = ["rev1", "rev5", "amihud", "volshock", "idiovol", "hlrange"]
...
def build_factors(df):
    ...
    del mr, mm, vr, vm, cv, beta, r, m, lr, s5, illiq, v, base
...
def main():
    ...
    res = {"criteria": "见 scripts/k6_spanning.py docstring（先于结果写下）",
           "specs": {}}
    y_all = S["long"]
    log("\n" + "=" * 74)
    for spec, cols in SPECS.items():
        ...
        pos, per_fold = 0, {}
        for fold in FOLDS:
            idx = S.index[S["fold"] == fold]
            dd = d.loc[d.index.intersection(idx)]
            if len(dd) < 40:
                continue
            bb, _ = nw_ols(dd["y"].to_numpy(),
                           np.column_stack([np.ones(len(dd)),
                                            dd.drop(columns=["y"]).to_numpy()]), 5)
            per_fold[fold] = float(bb[0] * 252 * 100)
            pos += int(bb[0] > 0)
    ...
    S.to_parquet(OUT / "k6_strategy_daily.parquet")
    C.to_parquet(OUT / "k6_control_daily.parquet")
    (OUT / "k6_spanning.json").write_text(
        json.dumps(res, indent=2, ensure_ascii=False, default=float), encoding="utf-8")
    log("\n写入 outputs/k6_spanning.json + k6_{strategy,control}_daily.parquet")
